...
@section('content')
                                ...
                                @foreach($quizFinal->questions as $index => $question)
                                    <div class="question-block mb-4 p-4 border border-primary rounded">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" id="question_{{ $index + 1 }}" name="questions[{{ $index + 1 }}][question]" class="form-control" value="{{ old('questions.' . ($index + 1) . '.question', $question->questions) }}" required>
                                        </div>
                                        @foreach($question->answers as $answerIndex => $answer)
                                            <div class="mb-3">
                                                <label for="answer_{{ $index + 1 }}_{{ $answerIndex }}" class="form-label">Đáp án {{ $answerIndex + 1 }}:</label>
                                                <input type="text" id="answer_{{ $index + 1 }}_{{ $answerIndex }}" name="questions[{{ $index + 1 }}][answers][{{ $answerIndex }}][answer]" class="form-control" value="{{ old('questions.' . ($index + 1) . '.answers.' . $answerIndex . '.answer', $answer->answer_text) }}" required>
    ...
    <script>
        // JavaScript code for dynamically adding and removing questions
            ...
            const questionHtml = `
                <div class="question-block mb-4 p-4 border border-primary rounded">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="answer_${questionCount}_0" name="questions[${questionCount}][answers][0][answer]" class="form-control" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="answer_${questionCount}_1" name="questions[${questionCount}][answers][1][answer]" class="form-control" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="answer_${questionCount}_2" name="questions[${questionCount}][answers][2][answer]" class="form-control" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="answer_${questionCount}_3" name="questions[${questionCount}][answers][3][answer]" class="form-control" required>
        ...
        function updateQuestionNumbers() {
            ...
            questionBlocks.forEach((block, index) => {
                ...
                block.querySelectorAll('label').forEach((label, labelIndex) => {
                    const isAnswer = label.getAttribute('for').startsWith('answer');
                    if (isAnswer) {
                        label.setAttribute('for', `answer_${questionNumber}_${labelIndex}`);
                        block.querySelector(`input[name^="questions[${index + 1}][answers][${labelIndex}][answer]"]`).setAttribute('name', `questions[${questionNumber}][answers][${labelIndex}][answer]`);
                        block.querySelector(`input[name^="questions[${index + 1}][answers][${labelIndex}][is_correct]"]`).setAttribute('name', `questions[${questionNumber}][answers][${labelIndex}][is_correct]`);
                    } else {
                        label.setAttribute('for', `question_${questionNumber}`);
                        block.querySelector(`input[name^="questions[${index + 1}][question]"]`).setAttribute('name', `questions[${questionNumber}][question]`);
                    }
                });
                block.querySelectorAll('input[id^="question_"]').forEach((input, inputIndex) => {
                    input.setAttribute('id', `question_${questionNumber}`);
                });
            });
        }
