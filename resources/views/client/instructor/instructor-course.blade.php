                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="settings-widget">
                                @if ($data->contains('status', 0))
                                    <div class="settings-inner-blk p-4 bg-light rounded shadow">
                                        <div class="sell-course-head comman-space text-center mb-4">
                                            <h3 class="text-warning display-4">Danh sách khóa học đang chỉnh sửa</h3>
                                        ...
                                        <div class="settings-tickets-blk course-instruct-blk table-responsive rounded shadow-sm p-3 bg-white">
                                            <table class="table table-striped table-hover ">
                                                <thead class="table-primary">
                                                    <tr>
                                                        <th style="background: #f1c232; background: linear-gradient(#f1c232, #ffff00);">Hình ảnh</th>
                                                        <th style="background: #f1c232; background: linear-gradient(#f1c232, #ffff00);">Video demo</th>
                                                        <th style="background: #f1c232; background: linear-gradient(#f1c232, #ffff00);">Tên khóa học</th>
                                                        <th style="background: #f1c232; background: linear-gradient(#f1c232, #ffff00);">Giá</th>
                                                        <th style="background: #f1c232; background: linear-gradient(#f1c232, #ffff00);">Hành động</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                ...
                                @if (!$data->contains('status', 0) && !$data->contains('status', 1) && !$data->contains('status', 2))
    <div class="settings-inner-blk p-4 bg-light rounded shadow text-center my-5">
        <h3 class="text-primary display-4 mb-4">Hiện bạn chưa có khóa học nào được tạo</h3>
        <p class="text-muted lead mb-4">Bạn có thể bắt đầu tạo khóa học ngay bây giờ và chia sẻ kiến thức của mình với mọi người.</p>
        <a href="{{ route('client.create-course') }}" class="btn btn-lg btn-primary shadow">
            <i class="fas fa-plus-circle"></i> TẠO KHÓA HỌC NGAY
        </a>
    </div>
@endif
                                ...
                                @if ($data->contains('status', 1))
                                    <div class="settings-inner-blk p-4 bg-light rounded shadow">
                                        <div class="sell-course-head comman-space text-center mb-4">
                                            <h3 class="text-success display-4">Danh sách khóa học đã được gửi duyệt</h3>
                                        ...
                                        <div class="settings-tickets-blk course-instruct-blk table-responsive rounded shadow-sm p-3 bg-white">
                                            <table class="table table-striped table-hover">
                                                <thead class="table-primary">
                                                    <tr>
                                                        <th style="background: #00ff00; background: linear-gradient(#00ff00, #289a16);">Hình ảnh</th>
                                                        <th style="background: #00ff00; background: linear-gradient(#00ff00, #289a16);">Video demo</th>
                                                        <th style="background: #00ff00; background: linear-gradient(#00ff00, #289a16);">Tên khóa học</th>
                                                        <th style="background: #00ff00; background: linear-gradient(#00ff00, #289a16);">Giá</th>
                                                        <th style="background: #00ff00; background: linear-gradient(#00ff00, #289a16);">Hành động</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach ($data as $post)
                                                                ...
                                                                <td>
                                                                    <div class="d-flex justify-content-between align-items-center">
                                                                        ...
                                                                        <form action="{{ route('client.recallCourse', $post->id) }}" method="POST" class="recall-course-form">
                                                                            @csrf
                                                                            @method('POST')
                                                                            <button type="button" class="btn btn-danger recall-course-btn">
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <div class="d-flex justify-content-between align-items-center">
                                                                        ...
                                                                        <form action="{{ route('client.deleteCourse', $post->id) }}" method="POST" class="delete-course-form">
                                                                            @csrf
                                                                            <button type="button" class="btn btn-outline-danger delete-course-btn rounded-pill">
                                                                                <i class="fas fa-trash-alt"></i> Xóa
                                                                            </button>
                                                                        </form>
                            ...
                            <div id="confirmSubmitModal" class="modal">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-buttons">
                                        <button type="button" class="btn btn-secondary"
                                            id="cancelSubmitBtn">Hủy</button>
                                        <button type="button" class="btn btn-success" id="confirmSubmitBtn">Gửi
                                            duyệt</button>
                                    </div>
                                </div>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.onclick = function(event) {
                if (event.target == deleteModal) {
                    deleteModal.style.display = "none";z
                }
